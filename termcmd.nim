...
import gintro/[gtk, glib, gobject, pango ,vte ]
import strformat
...
proc  init_Terminal() =
  ...
  terminal.setScrollOnOutput(true)                     #  défilement 

  terminal.setRewrapOnResize(true)                     #  Contrôle si le terminal remballera ou non son contenu,
...
proc winMmain() =
  ...
  #window.defaultSize = (800, 400)
  terminal = newTerminal()
  init_Terminal()
  let environ = getEnviron()
  let command = environ.environGetenv("SHELL")
  var pid = 0
  if not terminal.spawnSync({}, nil, [command], [], {}, nil, nil, pid, nil):
    echo "Terminal Error!"
    quit(QuitFailure)
